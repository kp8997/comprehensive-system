1. **Jumping into answering the question immediately**
    Don't jump into answering the question immediately! Take the time to gather requirements and clarify assumptions by asking questions.

2. **Approaching the question in an unstructured manner**
    You should have a systematic thinking framework to follow when answering system design questions.

3. **Insisting on only one solution or the best solution**

4. **Remaining silent the entire time**
    On the opposite of the spectrum of answering too quickly, there are people who remain too silent. Think out loud

5. **Going down a rabbit hole**
    Design the high level first, then proceed to elaborate into details step by step when needed.

6. **Using buzzwords without being able to explain them**
    Never drop a term you can't defend under follow-up questioning
